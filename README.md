...
- **按机器人参数**：参数页和机器人详情页会锁定机器人 ID；最大线速度、最大角速度和障碍膨胀半径保存到服务端 `backend/data/robot_settings.json`。在线且 Nav2 已就绪时立即更新该机器人命名空间内的控制器、恢复节点及局部/全局代价地图；离线时明确标记待应用，并在该机器人下次由本平台启动导航栈时自动载入。远端实体机若不是由此工作区启动，应在机器侧启动流程中挂载同一配置源。
