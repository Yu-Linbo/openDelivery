    ...
    transform_thread_(nullptr)
{
    ...
    transform_publish_period_ = declare_parameter<double>(
        "transform_publish_period", transform_publish_period_);
    map_update_interval_ = tf2::durationFromSec(declare_parameter<double>(
        "map_update_interval", tf2::durationToSec(map_update_interval_)));
    throttle_scans_ = declare_parameter<int>("throttle_scans", throttle_scans_);
    maxUrange_ = declare_parameter<double>("max_urange", maxUrange_);
    minimum_score_ = declare_parameter<double>("minimum_score", minimum_score_);
    linearUpdate_ = declare_parameter<double>("linear_update", linearUpdate_);
    angularUpdate_ = declare_parameter<double>("angular_update", angularUpdate_);
    temporalUpdate_ = declare_parameter<double>("temporal_update", temporalUpdate_);
    particles_ = declare_parameter<int>("particles", particles_);
    delta_ = declare_parameter<double>("resolution", delta_);
